...
const XlsxReader = (props) => {
    ...
    const processXlsxContent = useCallback(
        (workbook) => {
            // 預定義 metadata 工作表標頭
    ...
    const handleReadXlsx = useCallback(
        async (xlsxPath, xlsxPassword) => {
    ...
    const handleChooseFile = useCallback(
        () => {
            dialog.showOpenDialog({
                ...
                filters: [
                    { name: 'xlsx', extensions: ['xlsx'] }, // 只允許 xlsx 結尾檔案
                ],
            }).then((obj) => {
                // 取得路徑
                let filePath = obj.filePaths[0];
                setXlsxPath(filePath ? filePath : '');
        ...
        <div>
            ...
            {alertVisible && 
                <Alert variant={success ? 'success' : 'danger'} className="mb-2">
                    { success ? '檔案成功開啟 ' : xlsxPath === '' ? '請選擇檔案' : '請確認檔案或密碼是否正確，若無密碼則無須輸入密碼' } ... 此提示將於 5 秒鐘後隱藏
                </Alert>
            }
